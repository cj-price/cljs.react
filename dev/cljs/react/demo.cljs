...
(defnc SiteHeader
  [{:keys [pref on-pref on-open open]}]
    ...
    (Div {:className (use-sx [container {:display :flex :gap 2
      ...
      (Div {:className (use-sx {:display :flex :align-items :center :gap 1.5})}
        (Button {:type "button" :aria-label "Open navigation"
                 :aria-expanded (if open "true" "false") :onClick on-open
...
(defnc Page
  [{:keys [pref on-pref selected on-select open on-open on-close]}]
  (let [{:keys [view]} (first (filter #(= selected (:id %)) sections))]
    (Div {:className (use-sx {:min-height "100vh"
      ...
      (SiteHeader {:pref pref :on-pref on-pref :on-open on-open :open open})
